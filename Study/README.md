...
This folder contains all neccessary documents and files to replicate the study. The following sections will explain further details.
...
- **Group A**: Contains the documents used for group A
    ...
    - protocol: Protocol to be filled out by the superviser
...
- **measurements.ods**: Raw .ods file used to save the calculated results from the program evaluation
- **questionnaire.ods**: Raw .ods file used to save the evaluated questionnaires collected during the study
...
1) Distribute the tasks from the `Code` folder among the 10 participants following this table:
    ...
    The provided code contains all the neccessary files to perform the study on the respective task without further modification. This includes:
...
2) Conduct the study with each participant seperatly
...
3) Take the set of study documents of the group the participant belongs to
    ...
    5) Let participants edit their tasks. The editing phase is seperated into three time slots. Each time slot is 15 minutes long. After the time has run out. Participants were told to save their work and make sure that it is compileable. If the program was not compileable, participants were given up to 5 minutes, to clean up the program. After the time slot participants were instructed to switch to the next task. A preliminary switch of the task was allowed.
...
4) For each participant:
    ...
    3) Save the console ouput generated by the `parser.py` script to the `measurement.ods` file for the corresponding participant

    4) Evaluate the collected questionnaire of each participant and save it to the `questionnaire.ods` file